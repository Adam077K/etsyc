...
|-------------|---------|---------|
| `Etsyc` | Display name in agent prompts and brand voice | `Acme` |
| `etsyc` | lower-case slug used in env vars / urls | `acme` |
| `ETSYC` | UPPER prefix for env vars (e.g. `ACME_API_KEY`) | `ACME` |
| `ETSYC-` | Linear / Jira ticket prefix | `ACME-` |
| `etsyc.com` | Production domain | `acme.com` |
| `https://etsyc.com` | Marketing site URL (often a separate platform) | `https://acme.com` |
| `https://github.com/Adam077K/etsyc` | GitHub repo URL | `https://github.com/org/repo` |
...
| `.claude/hooks/*.sh` and `*.js` | Hooks run on every action. Read each one before enabling. `gsa-context-monitor.js` references env var `ETSYC_NO_AUTOCOMPACT` — set it or remove the check. |
