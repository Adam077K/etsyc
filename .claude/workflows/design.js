export const meta = {
  name: 'design',
  description: 'Etsyc T5 design workflow — generates N independent design variations from different angles, scores each with parallel design-critic judges against the brand bar, and synthesizes a winning direction (grafting the best ideas from runners-up). Optimizes for craft/quality, not speed.',
  phases: [
...
Honor the project's brand bar (accent color, type scale, spacing, motion budget) — load the project's brand/design-system skill if one exists. MANDATORY: before designing, Read and apply these global craft skills — ~/.claude/skills/{ui-typography,baseline-ui,frontend-design,12-principles-of-animation,design-audit,web-design-guidelines}/SKILL.md (project brand tokens win on conflict). Return concept, layout, rationale, risks.`
}
...
const judged = await pipeline(
  ...
  (variation) => variation
    ? agent(
        `Score this Etsyc design variation against the brand quality bar and the brief.
Brief: ${BRIEF}
...
const synthesis = await agent(
  `Synthesize the final Etsyc design direction for: ${TARGET}.
Brief: ${BRIEF}
