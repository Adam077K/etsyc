# GSA Agent System — Core
*Synced identically across every GSA project. Do not add project-specific content here — put that in the project's own `CLAUDE.md`.*
...
Skills load **on demand only** — never preload. Current skill count and category breakdown are project-specific — see the project's `CLAUDE.md`.
...
Vendor lock-in decisions (Mem0, etc.) and their review triggers are project-specific — see the project's `CLAUDE.md`.
