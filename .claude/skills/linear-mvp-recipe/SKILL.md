...
description: "Etsyc's locked Linear label vocabulary, sub-ticket creation pattern, and single-synthesis-comment style. Use whenever any CEO or C-suite agent interacts with Linear tickets, creates sub-tickets, or posts comments."
tags: [workflow, etsyc-specific, linear, orchestration]
...
```
Etsyc — Main (product work)        ← Adam and agents work here
Etsyc — War Room (orchestration)   ← Standing Routines post here, separate from product
Etsyc — Strategy (board meetings)  ← Board meeting outputs
```
