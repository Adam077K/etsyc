  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-line bg-ground/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-page items-center gap-6 px-6 py-4">
          ...
          {LINKS.map((l) => {
            ...
            return (
              <Link
                ...
                className={`border-b-2 pb-0.5 text-caption uppercase transition-colors duration-state ease-kol ${
                  active
        ...
        <Link
          href="/cart"
          className="rounded-pill border border-line bg-surface px-4 py-1.5 text-caption text-ink transition-colors duration-state ease-kol hover:bg-ground"
        >
          Cart{cartCount > 0 ? ` · ${cartCount}` : ""}
        </Link>
        <Link
          href="/sell"
          className="hidden rounded-pill border border-line bg-surface px-4 py-1.5 text-caption text-ink transition-colors duration-state ease-kol hover:bg-ground md:inline-block"
        >
          Sell on KOL
        </Link>
        <Link
          href="/welcome"
          className="rounded-pill bg-accent-cta px-4 py-1.5 text-caption font-semibold text-accent-ink transition-transform duration-tap ease-kol active:scale-[0.98]"
        >
          Sign in
        </Link>
      </div>
