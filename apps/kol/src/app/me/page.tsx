 ...
 *
 * Data seam: the buyer's own signals (follows, saves, prefs, onboarding) stay
 * local via `useKolSession`. The makers, products, orders and communities those
 * signals resolve against come from the LIVE data layer (getData → Supabase
 * when env is present, mock otherwise), read async inside a browser-only effect
 * (see src/app/page.tsx for the same seam). An empty database keeps every tab's
 * designed empty state honest — no fabricated saves, follows, or purchases.
 */
...
import type { Community, Maker, Order, Product } from "@/lib/data";
import { formatPrice, orderStages } from "@/lib/mock/db"; // pure formatters/labels only
...
export default function ProfilePage() {
  ...
  // `follows` lands after hydration (localStorage), so re-fetch communities when
  // the followed set changes — a newly followed maker's community then appears.
  const followKey = session.follows.join(",");
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const data = getData();
        const [makerList, productList, orders] = await Promise.all([
          data.listMakers(),
          data.listProducts(),
          data.listOrders(),
        ]);
        // Communities ride on follows — resolve one per followed maker, drop
        // makers that run none. Empty DB → no makers → no communities.
        const communityResults = await Promise.all(
          session.follows.map((slug) => data.getCommunity(slug).catch(() => null)),
        );
        if (!active) return;
        setState({
          status: "ready",
          data: {
            ...
            orders,
            communities: communityResults.filter((c): c is Community => c !== null),
          },
        });
      } catch {
    ...
    // followKey re-runs the effect when the followed set changes.
  }, [followKey, session.follows]);

  const data = state.status === "ready" ? state.data : null;

  const savedProducts: Product[] = data
    ? session.saves.flatMap((id) => {
        const p = data.products.get(id);
        return p ? [p] : [];
      })
    : [];
  const followedMakers: Maker[] = data
    ? session.follows.flatMap((slug) => {
        const m = data.makers.get(slug);
        return m ? [m] : [];
      })
    : [];
  const joinedCommunities: Community[] = data
    ? data.communities.filter((c) => session.follows.includes(c.makerSlug))
    : [];
  ...
  return (
    <main className="mx-auto w-full max-w-page px-6 pb-[var(--space-16)]">
      ...
      {/* ---- loading / error gates for the live-data tabs ---- */}
      {state.status === "loading" ? (
        <div className="pt-[var(--space-5)]">
          <Skeleton className="h-6 w-48" />
          <div className="mt-[var(--space-4)] grid grid-cols-1 gap-4 md:grid-cols-12">
            {SAVED_PATTERN.map((cell, i) => (
        ...
        <div className="mt-[var(--space-5)] rounded-lg border border-line bg-surface p-[var(--space-6)]">
          <p className="text-caption uppercase text-muted">Couldn&rsquo;t load your account</p>
          <p className="mt-2 max-w-measure text-body text-ink">
            Something went wrong reaching your saves and follows. Refresh the page to try again.
          </p>
        </div>
      ) : (
        <>
          ...
          {tab === "saved" && (
            ...
            >
              ...
              <p className="mt-1 max-w-measure text-body text-muted">
                Every save keeps its maker. Tap any piece to fall back into the world it came
                from.
              </p>
              ...
              {savedProducts.length === 0 ? (
                ...
                <div className="mt-[var(--space-4)] grid grid-cols-1 gap-4 md:grid-cols-12">
                  {savedProducts.map((p, i) => {
                    ...
                    const maker = data?.makers.get(p.makerSlug);
                    return (
          ...
          {tab === "communities" && (
            ...
            >
              ...
              <p className="mt-1 max-w-measure text-body text-muted">
                Membership rides on your follows — a community is a layer of a maker&rsquo;s
                world, not a separate app.
              </p>
              {joinedCommunities.length === 0 ? (
                ...
                <div className="mt-[var(--space-4)] max-w-xl divide-y divide-line rounded-lg border border-line bg-surface shadow-subtle">
                  {joinedCommunities.map((c) => {
                    const m = data?.makers.get(c.makerSlug);
                    return (
                      <Link
                        key={c.makerSlug}
          ...
          {tab === "purchases" && (
            ...
            >
              ...
              {orders.length === 0 ? (
                <div className="mt-[var(--space-4)] rounded-lg border border-dashed border-line bg-surface p-[var(--space-6)]">
                  <p className="font-display text-h3 text-ink">No purchases yet</p>
                  <p className="mt-2 max-w-measure text-body text-muted">
                    When you buy from a maker, the order lands here — and in full on your orders
                    page, where you watch it get made.
                  </p>
                  <Link
                    href="/"
                    className="mt-4 inline-block rounded-pill bg-accent px-6 py-2.5 text-caption uppercase text-accent-ink transition-transform duration-tap ease-kol hover:bg-accent/90 active:scale-[0.98]"
                ...
                <>
                  <div className="mt-[var(--space-4)] max-w-xl divide-y divide-line rounded-lg border border-line bg-surface shadow-subtle">
                    {orders.map((o) => {
                      const m = data?.makers.get(o.makerSlug);
                      return (
                        <Link
                          key={o.id}
                          href={`/orders/${o.id}`}
                        ...
                        >
                          <span>
                            <b className="text-ink">
                              {o.items
                                .map((it) => {
                                  const p = data?.products.get(it.productId);
                                  return `${p?.title ?? it.productId}${
                                    it.qty > 1 ? ` ×${it.qty}` : ""
                                  }`;
                                })
                                .join(", ")}
                            ...
                            <span className="mt-0.5 block text-caption text-muted">
                              {m?.name ?? o.makerSlug} · placed {o.placed} ·{" "}
                              {orderStages[o.stage]}
                            </span>
                          </span>
            ...
            >
              ...
              <div className="mt-[var(--space-4)] grid grid-cols-1 gap-4 lg:grid-cols-12">
                ...
                <div className="rounded-lg border border-line bg-surface p-[var(--space-4)] shadow-subtle lg:col-span-7">
                  ...
                  <p className="mt-2 max-w-measure text-body text-muted">
                    Off by default. If you turn this on, other people can see a small public page
                    of the makers and crafts you gravitate toward. Nothing is shared until you
                    flip the switch — and you choose what shows.
                  </p>
                  ...
                  <div className="mt-3 rounded-md border border-line bg-ground p-4">
                    <p className="text-caption uppercase text-muted">
                      Preview · what others would see{" "}
                      {tastePublic ? "(live now)" : "(if you turn it on)"}
                    </p>
                  ...
                  <label className="mt-4 flex min-h-11 cursor-pointer items-center gap-3">
                    ...
                    <span className="text-body text-ink">
                      ...
                      {tastePublic
                        ? " — others can see the preview above"
                        : " → flip to go public"}
                    </span>
                  </label>
                </div>
