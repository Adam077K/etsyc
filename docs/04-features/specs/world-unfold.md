...
**Hero persistence (load-bearing — amended 2026-07-21, CPO Ruling 1)**
- `unfold` is a **same-source transition**: the clip does not change. Given the persistent video is playing, when the world unfolds, then the SAME video element carries playback across `GROWN → WORLD_OPEN` — it MUST NOT unmount, pause, re-source, or re-buffer, and **it MUST NOT cross-fade to a second buffer**. An automated test MUST assert element identity persists and `paused` never flips true across the transition.
- Given the world unfolds, when the 900 ms choreography runs, then the Film Layer container node identity is the same node it was in `GROWN`, and moving footage of the maker is visible at every sampled frame of the unfold.
