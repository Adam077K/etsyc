...
3. **Inline `videoProfile` vs. reference (§5.4, §9) — RESOLVED with Workstream C 2026-07-20 (ADR-0001 OQ-2).** Aligned: canonical source-of-truth is the `video_profiles` table; the engine reads `videos`/`video_profiles` and ignores the config mirror. This pipeline emits clip **references** (`config.media.clips[].id` = `videos.id`), not inline profiles — the inline `videoProfile` in store-config.schema §2.3 is a config-side mirror only. Nothing blocks either workstream. *Residual: schema-doc §2.3 wording alignment — C has flagged it to the schema owner.*
