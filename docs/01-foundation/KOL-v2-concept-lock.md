...
| D16 | **8 "final missing features" folded in** (from `KOL_Final_Missing_Features.docx`) | **7 into MVP, 1 roadmap.** (1) **Two Shopping Modes** — add search/categories/filters/delivery, but results show makers-on-film and **open the maker's world, never a flat grid** (default stays the discovery feed). (2) **Proof of Product** — maker-*declared & shown* provenance (role, materials, process, location, partners, process media); NOT 3rd-party physical verification (that stays roadmap per D7). (3) **Ask the Maker** — public per-product Q&A, maker answers text/audio/video. (4) **Exactly What to Expect** — required structured product-info standard (dimensions/materials/variation/production time/shipping/care/repairs/returns/customization limits). (5) **Trustworthy Reviews** — verified-purchase + photo/video + variation + expectation-accuracy + maker responses. (6) **Guided Buyer–Maker Co-Creation** — **FULL** commissioning: brief → buyer↔maker messaging → shared drafts → revisions → approve → make (brings a messaging + draft-versioning system into MVP; Ask-the-Maker reuses it). (7) **Relationship-Based Ranking** — follow/save + purchase/question/project signals feed ranking (adds follow & save; extends the video/rank engine). (8) **Live Studio Sessions** — **ROADMAP v1.1** (heavy real-time streaming infra; doesn't gate the core recorded-video loop). Detail: feature-tree "Missing-Features Addendum". | hard (scope); reversible (individual tiers) |

> **On D16.** Two structural adds to note for Phase 4/5: (a) a **buyer↔maker messaging + draft-versioning system** (from full co-creation D16-6, reused by Ask-the-Maker) — a sizable new subsystem; (b) **search/browse** with maker-first results (D16-1). Both expand MVP scope meaningfully; CPO to integrate all 8 into the feature tree screens + state machine during Phase 5, database-engineer to add the tables (messages, questions, commissions, follows/saves, product_specs, review_media) in the Phase 4 schema.
