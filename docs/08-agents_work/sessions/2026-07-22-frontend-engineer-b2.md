...
Seams flagged: session/ring cookie names (`kol_session`, `kol_engine_ring`) must converge with B1a; FeedCard lacks `craftLine`/`focalPoint`; B1b wires `useGrow().grow(source, cardEl)` + `data-feed-card`; parent unmounts provider only after B3 claims the film.
Gates: typecheck 0 · lint clean · 649/649 (was 630; +19 tests in 2 suites, incl. the load-bearing persistence suite).
